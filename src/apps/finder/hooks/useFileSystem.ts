...
const UUID_MIGRATION_KEY = "irxos:indexeddb-uuid-migration-v1";
...
export function useFileSystem(
  ...
  const formatFileSystem = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("irxos:file-size-timestamp-sync-v1");
  ...
  useEffect(() => {
    const syncFileSizesAndTimestamps = async () => {
      ...
      const syncKey = "irxos:file-size-timestamp-sync-v1";
