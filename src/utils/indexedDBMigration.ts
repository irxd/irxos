...
const MIGRATION_KEY = "irxos:indexeddb-uuid-migration-v1";
const BACKUP_KEY = "irxos:indexeddb-backup";
...
async function backupDataBeforeMigration() {
  ...
  try {
    ...
    const db = await new Promise<IDBDatabase>((resolve, reject) => {
      const request = indexedDB.open("irxOS", 4); // Open version 4 explicitly
...
export async function migrateIndexedDBToUUIDs() {
  ...
  const currentDBVersion = await new Promise<number>((resolve) => {
    const request = indexedDB.open("irxOS");
    request.onsuccess = () => {
